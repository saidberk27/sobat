...
class MesafeSensoruOku(Node):

    def __init__(self):
        super().__init__('mesafe_sensor_oku_node')
        qos_profile = QoSProfile(depth=10)  # Configure desired QoS profile
        self.mesafe_sensor1 = self.create_subscription(Float32, '/mesafe_sensor1', self.mesafe1_callback, qos_profile=qos_profile)
        self.mesafe_sensor2 = self.create_subscription(Float32, '/mesafe_sensor2', self.mesafe2_callback, qos_profile=qos_profile)
        self.mesafe_sensor3 = self.create_subscription(Float32, '/mesafe_sensor3', self.mesafe3_callback, qos_profile=qos_profile)
        self.mesafe_sensor4 = self.create_subscription(Float32, '/mesafe_sensor4', self.mesafe4_callback, qos_profile=qos_profile)
        self.mesafe_sensor5 = self.create_subscription(Float32, '/mesafe_sensor5', self.mesafe5_callback, qos_profile=qos_profile)
...
def main(args=None):
    ...
    minimal_publisher = MesafeSensoruOku()
